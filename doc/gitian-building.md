...
*Setup instructions for a gitian build of KristaTech Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the KristaTech
...
to the KristaTech GitHub release page.
...
- [Building KristaTech Core](#building-kristatech-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/kristatech-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/kristatech-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/kristatech-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/kristatech-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/kristatech-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/kristatech-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bedri/gitian.sigs/) repository:
...
You can also mail the files to bedri (bedri@kristalteknoloji.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/kristatech-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/kristatech-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/kristatech-osx-*-build.assert
```
