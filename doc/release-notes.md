...
- **Dynamic Algorithm Mapping:** In Fallback Mode (Version 11), the puzzle hashing algorithm for each elected miner is determined dynamically per-block using an index mapping formula: `Hash(Seed_H || MinerPubKey_i) % 18`. In Standard Mode (Version 12), the index simplifies to `minerIndex % 13` to balance hash algorithm usage across the active validator set.
