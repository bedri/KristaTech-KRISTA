...
* **Güvenlik Kontrolü**: Özel ağlarda (Testnet ve Regtest), aktif masternode sayısı düşükse ağ, belirlenimci bir anahtar havuzuna (deterministic pool of keys) geri döner (falls back). Ayrıca, yerel test ortamını izole etmek için quorum üyeleri 12 yerel anahtar kimliği (local key IDs) kümesine göre filtrelenir.
