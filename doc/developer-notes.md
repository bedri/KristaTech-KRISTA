...
- [Development guidelines](#development-guidelines)
    - [General KristaTech Core](#general-kristatech-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
KristaTech Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
KristaTech Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bedri/KristaTech-KRISTA/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_kristatech
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_kristatech --log_level=test_suite
$ valgrind -v --leak-check=full src/kristatechd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_kristatech.coverage/index.html`.
```
...
However, in open source software such as KristaTech Core, where everyone uses
...
pay attention to for reviewers of KristaTech Core code.

General KristaTech Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: KristaTech Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against KristaTech Core so that they can be integrated
...
file descriptors it will cause problems with KristaTech's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:KristaTech-Project/KristaTech-KRISTA.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `kristatech-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `kristatech-cli` and the GUI debug console use this table to determine how to
