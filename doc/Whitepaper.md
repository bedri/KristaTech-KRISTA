...
|   1. Roll VRF Seed: Seed_H = Hash(Seed_{H-1} || VRF_Proof_{H-1})      |
...
Where $\text{VRF\_Proof}_{H-1}$ is a deterministic signature (RFC 6979) generated by the Coordinator of block $H-1$ using its private key on the seed of block $H-2$ ($\text{Seed}_{H-2}$). Because ECDSA signatures under RFC 6979 are strictly deterministic, the Coordinator cannot manipulate the signature value to skew the election of block $H+1$. The rolling seed is stored directly in the block header, making the history of selection entropy immutable and publicly verifiable.
...
Unlike Turing-complete execution environments (e.g., Solidity on EVM), MESCAL uses a declarative paradigm that compiles directly into stack-based Bitcoin Script (`CScript`) bytecode.
