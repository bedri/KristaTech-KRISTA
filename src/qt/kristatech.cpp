...
#include "qt/kristatech/splash.h"
#include "qt/kristatech/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("kristatech-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in kristatech.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in kristatech.qrc)
...
/** Class encapsulating KristaTech Core startup and shutdown.
...
/** Main KRISTA application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (KRISTATECHGUI)
...
{
    window = new KRISTATECHGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &KRISTATECHGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &KRISTATECHGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // KristaTech: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KRISTATECHGUI::handlePaymentRequest);
        connect(window, &KRISTATECHGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. KristaTech can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(kristatech_locale);
    Q_INIT_RESOURCE(kristatech);
    ...
    /// 6. Determine availability of data directory and parse kristatech.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("KristaTech"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("KristaTech"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // kristatech: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("KristaTech didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
