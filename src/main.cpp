...
/** Fees smaller than this (in uKRISTA) are considered zero fee (for relaying, mining and transaction creation)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.GetHash() != consensus.hashGenesisBlock &&
        ...
        if (!(fOfflineSync && !fFallbackMode)) {
            ...
            if (fCheckSig && !VerifyAdamVRFProof(adamSeed, block.vAdamVRFProof, signingCoordinator)) {
            ...
            if (fCheckSig && !VerifyAdamCoordinatorSig(block, signingCoordinator)) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // KristaTech: We use certain sporks during IBD, so check to see if they are
