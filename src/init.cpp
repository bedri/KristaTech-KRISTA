...
void PrepareShutdown()
{
    ...
    util::ThreadRename("kristatech-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), KRISTATECH_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), KRISTATECH_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all KRISTA specific functionality (Masternodes) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-bypasscoordsig", _("Bypass coordinator VRF proof and signature verification (default: 0)"));
...
{
    util::ThreadRename("kristatech-loadblk");
...
/** Sanity checks
 *  Ensure that KristaTech is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. KristaTech is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single KristaTech process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. KristaTech is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //KristaTech specific: spork DB's
                ...
                // KristaTech: load previous sessions sporks if we have them.
