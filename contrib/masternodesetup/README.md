...
# KristaTech Masternode Setup Guide
***
## Required
1) **KRISTA collateral value at current block** ([consult the collateral table](../../README.md#rewards-breakdown))
...
Remember to do `kristatech-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `kristatech-cli addnode "ip" onetry`

      Example:
		  kristatech-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `kristatech-cli getblockcount`, and if yes give it time now to catch last block number 
...
## Congratulations your KristaTech node it's running
